...
fn read_vertices(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
...
fn read_indices(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
...
fn read_normals(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
                -> Result<Option<FbxContainer<Vec3>>, FbxError> {
    if let Ok(layer_element_normal) = nodes.find(geom_node_handle, "LayerElementNormal") {
        Ok(Some(scene::make_vec3_container(nodes, layer_element_normal, "Normals")?))
    } else {
        Ok(None)
    }
}

fn read_tangents(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
                -> Result<Option<FbxContainer<Vec3>>, FbxError> {
    if let Ok(layer_element_tangent) = nodes.find(geom_node_handle, "LayerElementTangent") {
        Ok(Some(scene::make_vec3_container(nodes, layer_element_tangent, "Tangents")?))
    } else {
        Ok(None)
    }
}

fn read_binormals(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
                 -> Result<Option<FbxContainer<Vec3>>, FbxError> {
    if let Ok(layer_element_tangent) = nodes.find(geom_node_handle, "LayerElementBinormal") {
        Ok(Some(scene::make_vec3_container(nodes, layer_element_tangent, "Binormals")?))
    } else {
        Ok(None)
    }
}

fn read_uvs(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
            -> Result<Option<FbxContainer<Vec2>>, FbxError> {
    if let Ok(layer_element_uv) = nodes.find(geom_node_handle, "LayerElementUV") {
        Ok(Some(FbxContainer::new(nodes, layer_element_uv, "UV",  |attributes| {
...
fn read_materials(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer)
                  -> Result<Option<FbxContainer<i32>>, FbxError> {
    if let Ok(layer_element_material_node_handle) = nodes.find(geom_node_handle, "LayerElementMaterial") {
        Ok(Some(FbxContainer::new(nodes, layer_element_material_node_handle, "Materials", |attributes| {
...
impl FbxGeometry {
    pub(in crate::resource::fbx) fn read(geom_node_handle: Handle<FbxNode>, nodes: &FbxNodeContainer) -> Result<FbxGeometry, FbxError> {
        Ok(FbxGeometry {
    ...
    pub(in crate::resource::fbx) fn get_skin_data(&self, scene: &FbxScene)
        ...
        for &deformer_handle in self.deformers.iter() {
            for &sub_deformer_handle in scene.get(deformer_handle)
                .as_deformer()?.sub_deformers.iter() {
                let sub_deformer = scene.get(sub_deformer_handle)
                    .as_sub_deformer()?;
