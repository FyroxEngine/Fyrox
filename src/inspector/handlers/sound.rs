    ...
    if let FieldKind::Object(ref value) = args.value {
        match args.name.as_ref() {
            GenericSource::NAME => helper.do_scene_command(SetSoundSourceNameCommand::new(
                source_handle,
                value.cast_value().cloned().unwrap(),
            )),
            GenericSource::GAIN => {
            ...
            GenericSource::PLAY_ONCE => helper.do_scene_command(
                SetSoundSourcePlayOnceCommand::new(source_handle, *value.cast_value().unwrap()),
            ),
    ...
    if let FieldKind::Object(ref value) = args.value {
        match args.name.as_ref() {
            SpatialSource::RADIUS => {
                helper.do_scene_command(SetSpatialSoundSourceRadiusCommand::new(
                    source_handle,
                    *value.cast_value().unwrap(),
                ))
            }
            SpatialSource::POSITION => {
                helper.do_scene_command(SetSpatialSoundSourcePositionCommand::new(
                    source_handle,
                    *value.cast_value().unwrap(),
                ))
            }
            SpatialSource::MAX_DISTANCE => {
                helper.do_scene_command(SetSpatialSoundSourceMaxDistanceCommand::new(
                    source_handle,
                    *value.cast_value().unwrap(),
                ))
            }
            SpatialSource::ROLLOFF_FACTOR => {
                helper.do_scene_command(SetSpatialSoundSourceRolloffFactorCommand::new(
                    source_handle,
                    *value.cast_value().unwrap(),
                ))
            }
