// VAULT-ATOM-01: Blob Storage Interface
// Maps binary capsules to persistent memory pages via mmap.
...
impl BlobStorage {
    ...
    /// Evict the memory-mapped page without deleting the file.
    pub fn evict(&self, id: &MythId) {
        self.pages.write().unwrap().remove(&id.as_str());
    }

    /// Permanently delete the blob page from disk and memory.
    ...
    /// Returns the UUID string of every `.page` file found on disk.
